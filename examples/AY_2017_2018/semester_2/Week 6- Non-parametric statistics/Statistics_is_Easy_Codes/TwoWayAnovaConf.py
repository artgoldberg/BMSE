...
def twowayanova(grps):
	...
	factor_a_ss = weightedsumofsq([sum(x) / float(len(x)) for x in factor_a], [float(len(x)) for x in factor_a], total_mean)
	factor_b_ss = weightedsumofsq([sum(x) / float(len(x)) for x in factor_b], [float(len(x)) for x in factor_b], total_mean)
...
for line in infile:
	if line.startswith('>'):
		...
		samples[r][c] += list(map(float,line.split()))
infile.close()
...
print("We have", conf_interval * 100, "% confidence that the true F-statistic", end=' ')
