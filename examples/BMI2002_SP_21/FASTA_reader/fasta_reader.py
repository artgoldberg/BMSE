...
class FastaReader:
  ...
  VALID_EXTENSIONS = set(('fasta', 'fna', 'ffn', 'faa', 'frn'))
  ...
  def validate(self):
    ...
    # error if pathname wrong extension
    _, extension = os.path.splitext(self.pathname)
    if extension[1:] not in self.VALID_EXTENSIONS:
      raise ValueError(f"pathname '{self.pathname}' doesn't have "
         f"a FASTA suffix ({self.VALID_EXTENSIONS})")
  ...
  def read(self):
    ...
    while line_num < num_lines:
      ...
      if line_num+1 == num_lines or lines[line_num+1].startswith('>'):
      ...
      while line_num < num_lines and not lines[line_num].startswith('>'):
        ...
        # error if line is empty
        if not line:
          raise ValueError(f"{line_num + 1}: empty line")
